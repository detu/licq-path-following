function [x_init,y_init,info] = jpredictor_tr_wc(problem, p_init, p_final, x_init, y_init, delta_t, t_final, verbose_level)
%JPREDICTOR_TR_WC Summary of this function goes here
% Modification of JPREDICTOR_TR without constraint in LP problem
% 
% [OUTPUTARGS] = JPREDICTOR_TR_WC(INPUTARGS) Explain usage here
% 
% Examples: 
% 
...
% $Author: suwartad $	$Date: 2015/11/02 20:27:06 $	$Revision: 0.1 $
...
while (t < t_final)
    ...
    [y, qp_val, qp_exit, oqp, k_zero_tilde, g] = solve_qp(prob, p_init, x_init, y_init, step);
    ...
    if (qp_exit < 0); % QP is infeasible
        ...
        % solve LP
        [delta_l, lp_exit] = solve_lp(prob, oqp, y_init, step, y, k_zero_tilde, g);
...
%             if (rho > 0.75)
%                 p_init  = p_init + step;
%                 %p_init  = p_init/t_final + step;
%                 delta_t = min(gamma*delta_t, (t_final-t));
%                 %delta_t = min(gamma*delta_t, (t_final-t)/t_final);
%                 %x_init  = x_init + y;
%                 %y_init  = y_init + delta_l;
%                 %t       = t + delta_t;
%             end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [y, qp_val, qp_exit, oqp, k_zero_tilde, g] = solve_qp(prob, p, x_init, y_init, step)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [delta_l, lp_exit] = solve_lp(prob, oqp, y_init, step, y,  k_zero_tilde, g)
