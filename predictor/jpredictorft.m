function [x_init,y_init,info] = jpredictorft(problem, p_init, p_final, x_init, y_init, delta_t, t_final, verbose_level)
%JPREDICTORFT Summary of this function goes here
% 
% Modification of jpredictor with different QP problem, see eq. (3.2)
% from Slava paper page 486
% [OUTPUTARGS] = JPREDICTORFT(INPUTARGS) Explain usage here
% 
% Examples: 
% 
...
% $Author: detu $	$Date: 2015/10/31 21:44:02 $	$Revision: 0.1 $
...
function [y, qp_exit, oqp, k_zero_tilde] = solve_qp(prob, p, x_init, y_init, step)
    ...
    %[y, fval, qp_exit] = quadprog(H,f,A,b,Aeq,beq,[],[],[],options);
    %fval
end
...
%     [delta_l, ~, lp_exit] = linprog(f,A,b,Aeq,beq,lb,ub,[],optionslin);
    ...
    %[delta_lp, lpval, lp_exit] = linprog(-f,A,b,Aeq,beq,lb,ub,[],optionslin);
