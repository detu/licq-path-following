...
[F,w_gl_SP,~,w_opt_SS]  = WellSimulator(par);
...
% uSS = w_gl_SP;
% xSS = w_opt_SS(1:nx+nz);
% save xuSS.mat uSS xSS;
...
%     Uk_prev = U0;
    ...
    p      = MX.sym('p',nu);
    measNoise          = [noise(1:8,sim_k);zeros(30,1)];
    x0_measure         = [dx0;z0] + 0*measNoise;
    
%     g   = {g{:},[X0;Z0]};
%     lbg = [lbg;vertcat(dx0,z0)];
%     ubg = [ubg;vertcat(dx0,z0)];
    % g   = {g{:},[X0;Z0]-[dx0;z0]};
    ...
    for k = 0:par.N-1
        ...
        for j = 1:d
            ...
            %         %Jstate =(QmaxGL(1:nx,1).*(Xkj{j} - xSS(1:nx)))' * (Xkj{j} - xSS(1:nx));
            %         Jstate =(QmaxGL(1:nx+nz,1).*([Xkj{j};Zkj{j}] - xSS))' * ([Xkj{j};Zkj{j}] - xSS);
            %
            %         %Jecon  = qj;
            %         %L = -(w_to.^2) + 0.5.*sum((w_gl).^2);
            %         %Jecon = -(Zkj{j}(29).^2) + 0.5*sum((Uk).^2);
            %Jecon = -Zkj{j}(29) + sum(Uk);
            
            %J = J + Jcontrol + Jstate + Jecon + sum((Uk_prev - Uk).^2);
            %J = J + Jcontrol + Jstate + Jecon;
            %J = J + Jcontrol + Jstate + Jecon + (Uk_prev - Uk)'*(Uk_prev - Uk);
            %J = J + Jecon;
            %         J = J + Jcontrol + Jstate;
            
            
            %         if par.sv
            %             J   = J + (B(j+1)*qj*par.tf ) + 100000*s{j} + 10000.*sum((Uk_prev - Uk).^2) ;
            %         else
            %             J   = J + (B(j+1)*qj*par.tf ) + 10000.*sum((Uk_prev - Uk).^2) ; %
            %             %J   = J + (B(j+1)*qj*par.tf );
            %             %J   = J + (B(j+1)*qj*par.tf ) + sum((Uk_prev - Uk).^2) ;
            %         end
        end
...
%         ubg = [ubg;par.qGLMax];
    ...
    nlp     = struct('x',vertcat(w{:}),'p',p,'f',J,'g',vertcat(g{:}));
    solver  = nlpsol('solver','ipopt',nlp);
    
    GOR_true(:,sim_k)   = par.GOR;
    
    tic;
    sol         = solver('x0',w0,'p',GOR_true(:,sim_k),'lbx',lbw,'ubx',ubw,'lbg',lbg,'ubg',ubg);
    t(sim_k)    = toc;
    ...
    [u_nlp_opt, x_nlp_opt] = extractNLPSolution(w_opt);
    ...
    w_po1       = w_opt([(nx+nz-11),nu+(nx+nz-11)+(nx+nz+nu):d*(nx+nz+sv)+nx+nu:n_w_i]);
    w_po2       = w_opt([(nx+nz-10),nu+(nx+nz-10)+(nx+nz+nu):d*(nx+nz+sv)+nx+nu:n_w_i]);
    w_oEx       = w_opt([(nx+nz-1),nu+(nx+nz-1)+(nx+nz+nu):d*(nx+nz+sv)+nx+nu:n_w_i]);
    w_pg1       = w_opt([(nx+nz-13),nu+(nx+nz-13)+(nx+nz+nu):d*(nx+nz+sv)+nx+nu:n_w_i]);
    w_pg2       = w_opt([(nx+nz-12),nu+(nx+nz-12)+(nx+nz+nu):d*(nx+nz+sv)+nx+nu:n_w_i]);
    w_gEx       = w_opt([(nx+nz-0),nu+(nx+nz-0)+(nx+nz+nu):d*(nx+nz+sv)+nx+nu:n_w_i]);
    p_bh1       = w_opt([(nx+nz-23),nu+(nx+nz-23)+(nx+nz+nu):d*(nx+nz+sv)+nx+nu:n_w_i]);
    p_bh2       = w_opt([(nx+nz-22),nu+(nx+nz-22)+(nx+nz+nu):d*(nx+nz+sv)+nx+nu:n_w_i]);
    
    % implement the first sample on the simulator
    
    %u_in_1  = u_opt1(1) ;
    %u_in_2  = u_opt2(1);
    u_in_1  = u_nlp_opt(1,1);
    u_in_2  = u_nlp_opt(2,1);
    ...
    if PlotResult
        ...
        %plot(tgrid,par.QgMax.*ones(size(tgrid)),'k') %EKA
        plot(tgrid,xSS(38).*ones(size(tgrid)),'k')
        plot([0,0],[0,11],'k:')
