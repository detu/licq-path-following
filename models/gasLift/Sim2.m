...
for sim_k = 1:nIter
    ...
    GOR_real            = par.GOR + GOR_RN(:,sim_k).*par.GOR_var;
...
for sim_k = 1:nIter
    ...
    par.slip_real   = 1.5;
    par.GOR_real    = par.GOR + GOR_RN(:,sim_k).*par.GOR_var;
...
%%
IdRTO.GOR_true   = GOR_true;
IdRTO.GOR_est    = GOR_est;
IdRTO.par        = par;
IdRTO.meas       = meas;
IdRTO.NMPC       = NMPC;
IdRTO.sim_k      = sim_k;
...
tH      = (1:IdRTO.sim_k).*IdRTO.par.tf./3600; 
meas.tH = (1:IdRTO.sim_k*IdRTO.par.tf).*IdRTO.par.tSim./3600; 

figure(124)
clf
subplot(221)
stairs(meas.tH,IdRTO.meas.p_wh')
hold all
grid on
...
subplot(222)
hold all
plot(meas.tH,IdRTO.meas.w_tg)
grid on
ylabel('Oil rate [kg/s]','Interpreter','latex')

subplot(224)
hold all
plot(tH,IdRTO.GOR_true')
plot(tH,IdRTO.GOR_est','--')
grid on
ylabel('GOR [kg/kg]','Interpreter','latex')

subplot(223)
plot(meas.tH,IdRTO.meas.p_bh')
grid on
...
subplot(221)
hold all
% stairs(tH,IdRTO.NMPC.u_in_MPC(1,:))
% stairs(tH,IdRTO.NMPC.u_in_MPC(2,:))
stairs(tH,IdRTO.NMPC.u_in_RTO(1,:))
stairs(tH,IdRTO.NMPC.u_in_RTO(2,:))
stairs(tH,IdRTO.u_opt_ideal','--')
grid on
ylabel('GasLift rate rate [kg/s]','Interpreter','latex')
leg = legend('well 1 - ideal','well 2 - ideal','well 1 - SRTO','well 2 - SRTO');
